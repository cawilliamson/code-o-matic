...
mod tests {
    ...
    fn compact_threshold_respected() {
        ...
        // when: compact runs against a session below the message threshold
        let result = compact(&CompactionSettings::default(), "leaf", &ctx);
        // then: the call returns an error without compacting
    ...
    fn compact_produces_summary() {
        ...
        // when: compact runs against a session exceeding the message threshold
        let (before, after, entry) = compact(&CompactionSettings::default(), "leaf", &ctx).unwrap();
        // then: a compaction entry is appended covering the older messages
