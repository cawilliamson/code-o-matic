...
use code_o_matic::history::{History, Role};
...
fn system_prompt_prepended_once() {
    ...
    // when: to_request is called on a history with a system prompt
    let req = h.to_request("m", &[]);
    // then: the system message is prepended exactly once before the user and assistant messages
...
fn no_system_prompt_when_none() {
    ...
    h.append_user("hi".to_string());
    // when: to_request is called on a history with no system prompt
    let req = h.to_request("m", &[]);
    // then: only the user message is present
...
fn system_prompt_not_mutated_by_repeated_calls() {
    ...
    // when: to_request is called twice on the same history
    let _ = h.to_request("m", &[]);
    let _ = h.to_request("m", &[]);
    // then: the underlying messages vec never stores the system message
...
fn jsonl_roundtrip_preserves_messages() {
    ...
    // when: the history is serialized to jsonl and deserialized back
    let jsonl = h.to_jsonl().unwrap();
    let restored = History::from_jsonl(&jsonl).unwrap();
    // then: the restored history equals the original
