...
mod tests {
    ...
    fn cost_compute_gpt4o_mini_rates() {
        ...
        // when: CallCost::compute is called with gpt-4o-mini rates and a usage value
        let cost = CallCost::compute(&rates, &usage);
        let expected = 174.0 * 0.15e-6 + 10.0 * 0.6e-6;
        // then: the total cost and token counts match the rate card
    ...
    fn sse_parse_done_event() {
        // when: parse_sse_event is called with a data: [DONE] line
    ...
    fn sse_parse_content_delta() {
        ...
        // when: parse_sse_event is called with a content delta chunk
        let evts = parse_sse_event(raw);
        // then: a single delta-content event carrying the text is produced
        assert_eq!(evts.len(), 1);
        match &evts[0] {
    ...
    fn sse_parse_reasoning_and_content_same_chunk() {
        ...
        // when: parse_sse_event is called with a chunk carrying both reasoning and content
        let evts = parse_sse_event(raw);
        // then: both a reasoning event and a content event are produced in order
    ...
    fn sse_parse_tool_call_delta() {
        ...
        // when: parse_sse_event is called with a tool-call delta chunk
        let evts = parse_sse_event(raw);
