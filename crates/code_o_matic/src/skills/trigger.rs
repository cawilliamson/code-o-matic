...
mod tests {
    ...
    fn non_alphabetic_trigger_matches_substring() {
        // when: trigger_word_match is called with non-alphabetic triggers
        // then: triggers match by substring
    ...
    fn short_alpha_trigger_requires_both_boundaries() {
        // when: trigger_word_match is called with the short trigger 'pr'
        // then: matches require both word boundaries
    ...
    fn long_alpha_trigger_matches_start_boundary() {
        // when: trigger_word_match is called with the long trigger 'review'
        // then: matches require only a word-start boundary
    ...
    fn trigger_matching_is_case_insensitive() {
        // when: trigger_word_match is called with mixed-case text
        // then: matches succeed regardless of case
    ...
    fn scoped_match_includes_practice_and_domain() {
        ...
        // when: match_skills_scoped runs with rust in the domain scope
        let matched = match_skills_scoped("rust git", &skills, &scope);
        // then: both the domain and practice skills match
        ...
        // when: match_skills_scoped runs with an empty domain scope
        let matched = match_skills_scoped("rust git", &skills, &no_scope);
        // then: the practice skill still matches despite the empty scope
        assert_eq!(matched.len(), 2);
    }
}
