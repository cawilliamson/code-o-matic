...
pub enum SkillSource {
    ...
    External,
    /// from `.com/skills/` in the project.
    Project,
}
...
mod tests {
    ...
    fn skill_with_full_frontmatter_parses() {
        let raw = "---\nname: rust\ncategory: domain\ntrigger: [\"rust\", \"cargo\"]\npriority: 10\ntokens: 700\n---\nbody here\n";
        // when: the skill loader splits and parses the frontmatter
        let (front, body) = split_frontmatter(raw);
        let fm: Frontmatter = serde_yaml::from_str(front).unwrap();
        // then: all frontmatter fields and the body are recovered
    ...
    fn skill_with_no_frontmatter_defaults_to_name_and_domain() {
        // when: parse_from_memory reads a bare markdown file
        let skill = parse_from_memory("review.md", "review skills\n").unwrap();
        // then: the name is the file stem, category is domain, and a name-derived trigger exists
    ...
    fn skill_with_description_defaults_to_practice() {
        // when: parse_from_memory reads a file with a description but no category
        let skill =
            parse_from_memory("git.md", "---\ndescription: git helpers\n---\nuseful git tips\n")
                .unwrap();
        // then: the category defaults to practice and a name-derived trigger exists
