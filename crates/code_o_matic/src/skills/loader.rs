...
pub fn skill_dirs(extra_dirs: &[PathBuf]) -> Vec<(PathBuf, SkillSource)> {
    ...
    if let Some(home) = home_dir() {
        dirs.push((home.join(".com").join("skills"), SkillSource::Global));
    }
    // external
    for d in extra_dirs {
...
mod tests {
    ...
    fn project_overrides_global_on_name_collision() {
        ...
        // when: load_all_skills runs over the global then project directories
        ...
        // then: the project skill wins on name collision
    ...
    fn alphabetical_within_tier() {
        ...
        // when: load_from_dir scans the directory
        let skills = load_from_dir(dir.path(), SkillSource::External);
        let names: Vec<_> = skills.iter().map(|s| s.name.as_str()).collect();
        // then: skills are returned in alphabetical order
