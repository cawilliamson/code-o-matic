...
pub fn register_skill_commands(api: &mut Registry, state: CommandState) {
...
fn skill_create(ctx: &CommandContext, state: &CommandState) -> anyhow::Result<CommandResult> {
    ...
    let dir = if global {
        home_dir()
            .map(|h| h.join(".com").join("skills"))
            .ok_or_else(|| anyhow::anyhow!("could not resolve home directory"))?
    } else {
        PathBuf::from(".com").join("skills")
    };
...
mod tests {
    ...
    fn source_glyph_mapping() {
        // when: source_glyph is called for each source tier
        // then: each tier maps to its single-character glyph
    ...
    fn skill_show_not_found() {
        ...
        // then: the result message reports the skill was not found
